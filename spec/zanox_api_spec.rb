...
describe "Zanox API" do
  ...
  it "establishes a connection to zanox api and gets basic report for a month
  in the past, checks it for sanety" do
    report = ZanoxAPI::Report.basic(
      Date.parse(CONFIG['basic']['fromdate']),
      Date.parse(CONFIG['basic']['todate']))
    report.total.should == CONFIG['basic']['total']
  end

  it "establishes a connection to zanox api and gets one hopefully correct dataset" do
    report = ZanoxAPI::Report.sales(Date.parse(CONFIG['sales']['date']))
  ...
  it "accepts abitrarily nested method invokations on response object" do
    ...
    response = ZanoxAPI::Response.new({:a => 1, :b => 2})
    response.a.should == 1
    response.b.should == 2
  ...
  it "processes data correctly" do
      ...
      response = ZanoxAPI::Response.new(api_response)
